...
- Laisse l'utilisateur écrire le code lui-même (entités, contrôleurs, templates, formulaires). Ne crée/modifie pas ces fichiers à sa place sauf demande explicite — tu peux en revanche lui indiquer où et quoi modifier.
...
- Projet créé avec le Symfony CLI (`symfony new . --webapp`), Symfony 7, pack `webapp` (Twig, Doctrine, Forms, Security, Mailer inclus).
...
- Routing par attributs PHP (`#[Route(...)]`) exclusivement — pas d'annotations ni de configuration YAML pour les routes.
...
- Contrôleurs dans `src/Controller/`, un contrôleur par entité principale (`BookController`, `AuthorController`...), routes nommées avec le préfixe `app_` (`app_book_index`, `app_book_show`...).
- Templates Twig dans `templates/`, un layout de base (`base.html.twig`) étendu par les autres.
- Formulaires via Symfony Forms (`src/Form/`), validation via les contraintes `Assert` posées sur l'entité plutôt qu'à la main dans le contrôleur.
